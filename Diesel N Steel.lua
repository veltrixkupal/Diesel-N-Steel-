...
local existingHub = CoreGui:FindFirstChild("Kryzen.net") or 
                    LocalPlayer.PlayerGui:FindFirstChild("Kryzen.net")
...
local function ApplyGradientToUI()
    ...
    local function ApplyGradient(v)
        if v:IsA("UIStroke") then
            ...
            local grad = v:FindFirstChild("KryzenGrad") or Instance.new("UIGradient")
            grad.Name = "KryzenGrad"
    ...
    for _, v in pairs(PlayerGui:GetDescendants()) do ApplyGradient(v) end
    PlayerGui.DescendantAdded:Connect(function(v) ApplyGradient(v) end)
end
...
local Misc = Window:AddTab({ Title = "Misc" })
local Roles = Window:AddTab({ Title = "Roles" })
local Music = Window:AddTab({ Title = "Music" })
local Boost = Window:AddTab({ Title = "Boost" })
local TP = Window:AddTab({ Title = "TP" })
local Troll = Window:AddTab({ Title = "Troll" })

local ExpSection = Home:AddSection({ Title = "EXP" })
...
local KmSection = Home:AddSection({ Title = "Km" })
...
local DuperSection = Home:AddSection({ Title = "Duper" })
...
MainGradient.Color = ColorSequence.new{
    ...
    ColorSequenceKeypoint.new(1.00, Color3.fromRGB(230, 230, 255))
}
...
BorderGradient.Color = ColorSequence.new{
    ColorSequenceKeypoint.new(0.00, Color3.fromRGB(255, 255, 255)),
    ColorSequenceKeypoint.new(0.50, Color3.fromRGB(230, 230, 255)),
...
Title.Text = "Kryzen.net Dupe Cash"
...
ButtonBorderGradient.Color = ColorSequence.new{
    ColorSequenceKeypoint.new(0.00, Color3.fromRGB(255, 255, 255)),
    ColorSequenceKeypoint.new(0.50, Color3.fromRGB(230, 230, 255)),
    ColorSequenceKeypoint.new(1.00, Color3.fromRGB(255, 255, 255))
}
    ...
    for i = 1, 950 do
        local thread = task.spawn(function()
            while dupeEnabled do
                pcall(function() catNet:FireServer(dupeArgs) end)
                task.wait(0.25)
            end
        end)
...
local DeductSection = Home:AddSection({ Title = "Deduct" })

local deductValue = 0
local autoRemoveRunning = false
local autoRemoveLoop = nil

DeductSection:AddSlider({
...
local ResetSection = Home:AddSection({ Title = "Reset" })
...
local AutoCashSection = Home:AddSection({ Title = "Auto Cash" })
...
AutoCashSection:AddToggle({
    Title = "Auto Cash",
    Description = "Need 2k to Get Inf Cash",
    Default = false,
    Callback = function(state)
                    ...
                    pcall(function()
                        remotes:WaitForChild("CloseCustomize", 9e9):FireServer({
                            ...
                            ["NewOwnedParts"] = {
                                ...
                                ["CL - 02"] = 100, ["TO - 01"] = 100, ["4HK1 Twin Turbo"] = 100,
                                ["4JJ1"] = 100, ["4HK1 Single Turbo"] = 100, ["4BC2"] = 100,
                                ["4HE1 Single Turbo"] = 100, ["4-Speed Manual (High Ratio)"] = 100,
                                ["T - 01 (F)"] = 100, ["EO - 01"] = 100, ["T - 05 (R)"] = 100,
                                ["T - 03 (R)"] = 100, ["TO - 03"] = 100, ["EO - 03"] = 100,
                                ["TO - 05"] = 100, ["EO - 05"] = 100, ["4HF1 Twin Turbo"] = 100,
                                ["TO - 02"] = 100, ["C - 04"] = 100, ["BA - 02"] = 100,
                                ["EO - 04"] = 100, ["T - 04 (R)"] = 100, ["C - 02"] = 100,
                                ["BA - 04"] = 100, ["T - 02 (F)"] = 100, ["EO - 02"] = 100,
                                ["B - 05"] = 100, ["R - 01"] = 100, ["BF - 02"] = 100,
                                ["C - 03"] = 100, ["B - 03"] = 100, ["BF - 01"] = 100,
                                ["T - 05 (F)"] = 100, ["4-Speed Manual"] = 100, ["B - 04"] = 100,
                                ["TO - 04"] = 100, ["4JK1"] = 100, ["CL - 01"] = 100,
                                ["T - 01 (R)"] = 100, ["R - 02"] = 100, ["B - 02"] = 100,
                                ["4BE1"] = 100, ["T - 04 (F)"] = 100, ["B - 01"] = 100,
                                ["T - 03 (F)"] = 100, ["D - 01"] = 100, ["C - 01"] = 100,
                            },
                            ["NewPartsStatus"] = {
                            ...
                            ["NewEquippedParts"] = {
                                ...
                                ["Battery"] = "BA - 01", ["Transmission"] = "4-Speed Manual",
                                ["Coolant"] = "C - 01", ["TransmissionOil"] = "TO - 01",
                                ["RearTires"] = "T - 01 (R)", ["Radiator"] = "R - 01",
                                ["BrakeFluid"] = "BF - 01", ["FrontTires"] = "T - 01 (F)",
                                ["EngineOil"] = "EO - 01", ["Engine"] = "4BC2",
                            },
                        })
                    end)
                    task.wait()
...
local SendSection = Home:AddSection({ Title = "Send" })
...
local JeepSpawnSection = Home:AddSection({ Title = "Jeep Spawner" })

local selectedSpawnJeep = nil
local selectedSpawnGarage = nil
local garageList = {}
...
local function getJeepList()
    ...
    if jeepnies then
        for _, jeep in pairs(jeepnies:GetChildren()) do
            if jeep:IsA("Model") or jeep:IsA("Folder") then
                table.insert(jeepNames, jeep.Name)
            end
        end
    end
    if #jeepNames == 0 then
        jeepNames = {"No Jeeps Found"}
    end
    return jeepNames
end

local function getGarageList()
    local garages = workspace:WaitForChild("Map", 9e9):WaitForChild("Misc", 9e9):WaitForChild("Garages", 9e9)
    local garageNames = {}
    for _, garage in pairs(garages:GetChildren()) do
        table.insert(garageNames, garage.Name)
        garageList[garage.Name] = garage
    end
    if #garageNames == 0 then
        garageNames = {"No Garages Found"}
    end
    return garageNames
end

JeepSpawnSection:AddDropdown({
    Title = "Select Jeep",
    Description = "Choose which jeep to spawn",
    Values = getJeepList(),
    Default = "Select Jeep",
    Callback = function(value)
        if value and value ~= "Select Jeep" and value ~= "No Jeeps Found" then
            selectedSpawnJeep = value
        end
    end
})
...
JeepSpawnSection:AddDropdown({
    Title = "Select Garage",
    Description = "Choose garage to spawn at",
    Values = getGarageList(),
    Default = "Select Garage",
    Callback = function(value)
        if value and value ~= "Select Garage" and value ~= "No Garages Found" then
            selectedSpawnGarage = garageList[value]
        end
    end
})

JeepSpawnSection:AddButton({
    Title = "Refresh Jeeps",
    Description = "Refresh the jeep list",
    Callback = function()
        local newList = getJeepList()
    end
})

JeepSpawnSection:AddButton({
    Title = "Refresh Garages",
    Description = "Refresh the garage list",
    Callback = function()
        local newList = getGarageList()
    end
})

JeepSpawnSection:AddButton({
    Title = "Spawn Jeep",
    Description = "Make sure you own this jeep before spawning",
    Callback = function()
        if not selectedSpawnJeep or selectedSpawnJeep == "Select Jeep" or selectedSpawnJeep == "No Jeeps Found" then
            return
        end
        if not selectedSpawnGarage then
            return
        end
        remotes:WaitForChild("SpawnJeepney", 9e9):FireServer({
            ["Garage"] = selectedSpawnGarage,
            ["Password"] = 643531260,
            ["JeepneyName"] = selectedSpawnJeep,
            ["Route"] = "Balagtas - Bulakan",
        })
    end
})
...
local function buyJeepWithUpgrade(jeepName, indexNumber, password)
    ...
    remotes:WaitForChild("CloseCustomize", 9e9):FireServer({
        ...
        ["NewOwnedParts"] = {
            ...
            ["4HE1 Single Turbo"] = 100, ["4-Speed Manual (High Ratio)"] = 100,
            ["T - 01 (F)"] = 100, ["EO - 01"] = 100, ["T - 05 (R)"] = 100,
            ["T - 03 (R)"] = 100, ["EO - 05"] = 100, ["T - 04 (R)"] = 100,
            ["T - 02 (R)"] = 100, ["R - 02"] = 100, ["TO - 05"] = 100,
            ["TO - 04"] = 100, ["TO - 03"] = 100, ["BA - 02"] = 100,
            ["EO - 04"] = 100, ["B - 02"] = 100, ["C - 02"] = 100,
            ["BA - 04"] = 100, ["T - 02 (F)"] = 100, ["EO - 02"] = 100,
            ["T - 04 (F)"] = 100, ["R - 01"] = 100, ["TO - 02"] = 100,
            ["T - 03 (F)"] = 100, ["B - 03"] = 100, ["BF - 01"] = 100,
            ["T - 05 (F)"] = 100, ["TO - 01"] = 100, ["B - 05"] = 100,
            ["CL - 01"] = 100, ["4BC2"] = 100, ["CL - 02"] = 100,
            ["T - 01 (R)"] = 100, ["BF - 02"] = 100, ["C - 03"] = 100,
            ["4BE1"] = 100, ["4HK1 Twin Turbo"] = 100, ["B - 01"] = 100,
            ["4HF1 Twin Turbo"] = 100, ["D - 01"] = 100, ["C - 01"] = 100,
        },
        ["NewPartsStatus"] = {
            ["FrontTiresHealth"] = 100, ["DifferentialHealth"] = 100, ["ClutchHealth"] = 100,
            ["TransmissionHealth"] = 100, ["RadiatorHealth"] = 100, ["CoolantLevel"] = 100,
            ["BrakeHealth"] = 100, ["EngineHealth"] = 100, ["RearTiresHealth"] = 100,
            ["BrakeFluid"] = 100, ["TransmissionOil"] = 100, ["EngineOil"] = 100, ["BatteryHealth"] = 100,
        },
...
local ShopSection = Shop:AddSection({ Title = "Jeepney Shop" })

ShopSection:AddButton({
    Title = "Sarao Custombuilt",
    Description = "Buy and fully upgrade Sarao Custombuilt Jeepney",
    Callback = function() buyJeepWithUpgrade("Sarao Custombuilt Model 2", 1, 800584595) end
})

ShopSection:AddButton({
    Title = "DF Devera Long",
    Description = "Buy and fully upgrade DF Devera Long Model",
    Callback = function() buyJeepWithUpgrade("DF Devera Long Model", 1, 800584595) end
})

ShopSection:AddButton({
    Title = "Morales 10 Seater",
    Description = "Buy and fully upgrade Morales 10 Seater",
    Callback = function() buyJeepWithUpgrade("Morales 10 Seater", 1, 800584595) end
})

ShopSection:AddButton({
    Title = "Milwaukee 11 Seater",
    Description = "Buy and fully upgrade Milwaukee Motor Sport",
    Callback = function() buyJeepWithUpgrade("Milwaukee Motor Sport 11 Seater", 1, 800584595) end
})

ShopSection:AddButton({
    Title = "XLT AUV 12 Seater",
    Description = "Buy and fully upgrade XLT AUV 12 Seater",
    Callback = function() buyJeepWithUpgrade("XLT AUV 12 Seater", 1, 284520541) end
})

local ToolsSection = Shop:AddSection({ Title = "Tools" })

local function buyTool(toolName)
    remotes:WaitForChild("BuyTool", 9e9):InvokeServer({
        [1] = { ["Password"] = 520430635, ["ToolName"] = toolName }
    })
end

ToolsSection:AddButton({ Title = "Diesel Can", Description = "Buy Diesel Can", Callback = function() buyTool("Diesel can") end })
ToolsSection:AddButton({ Title = "Wrench", Description = "Buy Wrench", Callback = function() buyTool("Wrench") end })
ToolsSection:AddButton({ Title = "Baseball Bat", Description = "Buy Baseball Bat", Callback = function() buyTool("Baseball bat") end })
ToolsSection:AddButton({ Title = "Metal Pipe", Description = "Buy Metal Pipe", Callback = function() buyTool("Metal pipe") end })
ToolsSection:AddButton({ Title = "Hammer", Description = "Buy Hammer", Callback = function() buyTool("Hammer") end })
ToolsSection:AddButton({ Title = "Coolant Can", Description = "Buy Coolant Can", Callback = function() buyTool("Coolant can") end })

local FoodSection = Shop:AddSection({ Title = "Food" })

local function buyFood(foodName)
    remotes:WaitForChild("BuyFood", 9e9):InvokeServer({
        [1] = { ["Password"] = 520430635, ["FoodName"] = foodName }
    })
end

FoodSection:AddButton({ Title = "Hotdog", Description = "Buy Hotdog", Callback = function() buyFood("Hotdog") end })
FoodSection:AddButton({ Title = "Water", Description = "Buy Water", Callback = function() buyFood("Water") end })
FoodSection:AddButton({ Title = "Fried Chicken", Description = "Buy Fried Chicken", Callback = function() buyFood("Fried Chicken") end })
FoodSection:AddButton({ Title = "Bloxy Cola", Description = "Buy Bloxy Cola", Callback = function() buyFood("Bloxy Cola") end })
FoodSection:AddButton({ Title = "Betamax", Description = "Buy Betamax", Callback = function() buyFood("Betamax") end })
FoodSection:AddButton({ Title = "Quek Quek", Description = "Buy Quek Quek", Callback = function() buyFood("Quek Quek") end })
FoodSection:AddButton({ Title = "Isaw", Description = "Buy Isaw", Callback = function() buyFood("Isaw") end })

local CpcSection = Misc:AddSection({ Title = "CPC Application" })

CpcSection:AddButton({
    Title = "Apply Guiguinto - Bulakan",
    Description = "Apply for CPC on Guiguinto route",
    Callback = function() 
        remotes:WaitForChild("ApplyForCPC", 9e9):FireServer({ [1] = { ["Route"] = "Guiguinto - Bulakan" } })
    end
})
...
CpcSection:AddButton({
    ...
    Callback = function() 
        remotes:WaitForChild("ApplyForCPC", 9e9):FireServer({ [1] = { ["Route"] = "Balagtas - Bulakan" } })
    end
})
...
CpcSection:AddButton({
    ...
    Callback = function() 
        remotes:WaitForChild("ApplyForCPC", 9e9):FireServer({ [1] = { ["Route"] = "Malolos - Bulakan" } })
    end
})

local RemoverSection = Misc:AddSection({ Title = "Remover" })

RemoverSection:AddButton({
...
local MessSection = Misc:AddSection({ Title = "Mess" })

MessSection:AddButton({
    Title = "Get Licence",
...
local ServerSection = Misc:AddSection({ Title = "Server" })
...
local BarkSection = Misc:AddSection({ Title = "Bark" })
...
local UnlockSection = Misc:AddSection({ Title = "Unlock All Parts" })

local selectedUnlockJeep = "Sarao Custombuilt Model 2"
local unlockJeepIndex = "1"
local unlockJeepOptions = { "Sarao Custombuilt Model 2", "DF Devera Long Model", "Morales 10 Seater", "Milwaukee Motor Sport 11 Seater", "XLT AUV 12 Seater" }

UnlockSection:AddDropdown({
    Title = "Select Jeep",
    Description = "Choose which jeep to unlock parts for",
    Values = unlockJeepOptions,
...
local LawSection = Roles:AddSection({ Title = "Law Enforcement" })
LawSection:AddButton({ Title = "Police", Description = "Spawn as Police", Callback = function() spawnRole("Police") end })
LawSection:AddButton({ Title = "Fire Enforcement", Description = "Spawn as Fire Enforcement", Callback = function() spawnRole("Fire Enforcement") end })

local TransportSection = Roles:AddSection({ Title = "Transportation" })
TransportSection:AddButton({ Title = "Driver", Description = "Spawn as Driver", Callback = function() spawnRole("Driver") end })
TransportSection:AddButton({ Title = "Conductor", Description = "Spawn as Conductor", Callback = function() spawnRole("Conductor") end })
TransportSection:AddButton({ Title = "Barker", Description = "Spawn as Barker", Callback = function() spawnRole("Barker") end })
TransportSection:AddButton({ Title = "Operator", Description = "Spawn as Operator", Callback = function() spawnRole("Operator") end })

local ManagementSection = Roles:AddSection({ Title = "Management" })
ManagementSection:AddButton({ Title = "Owner", Description = "Spawn as Owner", Callback = function() spawnRole("Owner") end })
ManagementSection:AddButton({ Title = "Co Owner", Description = "Spawn as Co Owner", Callback = function() spawnRole("Co Owner") end })
ManagementSection:AddButton({ Title = "Manager", Description = "Spawn as Manager", Callback = function() spawnRole("Manager") end })

local CivSection = Roles:AddSection({ Title = "Civilians" })
CivSection:AddButton({ Title = "Player", Description = "Spawn as Player", Callback = function() spawnRole("Player") end })
CivSection:AddButton({ Title = "Civilian", Description = "Spawn as Civilian", Callback = function() spawnRole("Civilian") end })
CivSection:AddButton({ Title = "Passenger", Description = "Spawn as Passenger", Callback = function() spawnRole("Passenger") end })
CivSection:AddButton({ Title = "VIP", Description = "Spawn as VIP", Callback = function() spawnRole("VIP") end })
...
function MusicController:Play(assetId)
    if self.currentSound then
        pcall(function() self.currentSound:Stop() self.currentSound:Destroy() end)
    end
...
local PlaylistSection = Music:AddSection({ Title = "Playlist" })
...
PlaylistSection:AddButton({ Title = "Play Selected Song", Description = "Play the selected song", Callback = function() MusicController:Play(selectedSongId) end })
PlaylistSection:AddButton({ Title = "Stop Music", Description = "Stop music", Callback = function() MusicController:Stop() end })

local SpeedSection = Boost:AddSection({ Title = "Speed" })
...
local TerminalsSection = TP:AddSection({ Title = "Terminals" })
TerminalsSection:AddButton({ Title = "Bulakan Terminal", Description = "Bulakan - Balagtas", Callback = function() teleportTo(Vector3.new(-626, 16, -3202)) end })
TerminalsSection:AddButton({ Title = "Balagtas Terminal", Description = "Balagtas - Bulakan", Callback = function() teleportTo(Vector3.new(-3922, 17, 3156)) end })
TerminalsSection:AddButton({ Title = "Malolos Terminal", Description = "Malolos - Bulakan", Callback = function() teleportTo(Vector3.new(17606, 16, -1195)) end })
TerminalsSection:AddButton({ Title = "Guiguinto Terminal", Description = "Guiguinto - Bulakan", Callback = function() teleportTo(Vector3.new(1060, 16, 3167)) end })

local DropSection = TP:AddSection({ Title = "Drop Off" })
DropSection:AddButton({ Title = "Drop point Bulakan - Guiguinto", Description = "Bulakan to Guiguinto drop point", Callback = function() teleportTo(Vector3.new(1049.858, 14.004, 3246.740)) end })
DropSection:AddButton({ Title = "Drop point Guiguinto - Bulakan", Description = "Guiguinto to Bulakan drop point", Callback = function() teleportTo(Vector3.new(-1545, 13, -3470)) end })
DropSection:AddButton({ Title = "Drop point Bulakan - Malolos", Description = "Bulakan to Malolos drop point", Callback = function() teleportTo(Vector3.new(17793, 13, -1080)) end })
DropSection:AddButton({ Title = "Drop point Bulakan - Balagtas", Description = "Bulakan to Balagtas drop point", Callback = function() teleportTo(Vector3.new(-3802, 13, 3357)) end })
DropSection:AddButton({ Title = "Drop point Balagtas - Bulakan", Description = "Balagtas to Bulakan drop point", Callback = function() teleportTo(Vector3.new(-1512, 13, -3471)) end })

local LocationsSection = TP:AddSection({ Title = "Locations" })
LocationsSection:AddButton({ Title = "Talyer", Description = "Teleport to Talyer", Callback = function() teleportTo(Vector3.new(-430.981, 12.701, 620.724)) end })
LocationsSection:AddButton({ Title = "Police Station", Description = "Teleport to Police Station", Callback = function() teleportTo(Vector3.new(1240.597, 12.863, 3211.784)) end })
LocationsSection:AddButton({ Title = "Malolos", Description = "Teleport to Malolos", Callback = function() teleportTo(Vector3.new(17796, 13, -1104)) end })
LocationsSection:AddButton({ Title = "Balagtas", Description = "Teleport to Balagtas", Callback = function() teleportTo(Vector3.new(-3879, 14, 3482)) end })
LocationsSection:AddButton({ Title = "Guiguinto", Description = "Teleport to Guiguinto", Callback = function() teleportTo(Vector3.new(822, 13, 3290)) end })
LocationsSection:AddButton({ Title = "Junk Shop", Description = "Teleport to Junk Shop", Callback = function() teleportTo(Vector3.new(-467, 13, 772)) end })
LocationsSection:AddButton({ Title = "Bulakan", Description = "Teleport to Bulakan", Callback = function() teleportTo(Vector3.new(-1455, 13, -3438)) end })

local FlingerSection = Troll:AddSection({ Title = "Jeep Flinger" })
...
FlingerSection:AddButton({
    ...
    Callback = function()
        if not selectedFlingTarget or selectedFlingTarget == "" or selectedFlingTarget == "Select Player" then
            return
        end
...
local HoodSection = Troll:AddSection({ Title = "Troll Hood" })
...
HoodSection:AddDropdown({
    Title = "Select Jeep",
    Description = "Choose which jeep to troll",
    Values = getJeepListTroll(),
    Default = "Select Jeep",
    Callback = function(value) 
...
HoodSection:AddButton({
    ...
    Callback = function()
        if not selectedHoodJeep or selectedHoodJeep == "" or selectedHoodJeep == "Select Jeep" then
            return
        end
...
local BreathingSection = Troll:AddSection({ Title = "Jeep Breathing" })

local breathingActive = false
local breathingLoop = nil
local breathingArgs = {
    ["Fuel"] = 49.93502950764896, ["RPM"] = 0, ["DifferentialHealth"] = 99.99882781420766,
    ["SteerC"] = 0, ["TransmissionHealth"] = 99.37959134382187, ["Throttle"] = 0.05,
    ["Mileage"] = 5, ["BatteryHealth"] = 99.9995, ["Gear"] = 0, ["DeductedExp"] = 0,
    ["Crashed"] = true, ["OilPressure"] = 21.75937747796297, ["FrontTiresHealth"] = 99.9994527327599,
    ["BrakeFluid"] = 99.99992500000002, ["TransmissionOil"] = 99.99626049976072, ["IsOn"] = false,
    ["ClutchHealth"] = 99.99998950000003, ["Brake"] = 0, ["RadiatorHealth"] = 99.99998868312831,
    ["CoolantLevel"] = 99.99888999999965, ["BrakeHealth"] = 99.99987500000002, ["RearTiresHealth"] = 99.9994527327599,
    ["EngineHealth"] = 99.98528651746355, ["EngineTemp"] = 46.295844052814054, ["Speed"] = 0.004640357103198767,
    ["EngineOil"] = 99.99626049976072, ["SteerT"] = 0, ["Password"] = 157913333,
}

BreathingSection:AddToggle({
...
pcall(function()
    local spyKeywords = { "Block remote", "Clear logs", "Copy code", "Get result", "Ignore remote", "Unblock all remotes", "Remote Spy", "Spy", "RemoteSpy" }
    local function ScanUI(obj)
        if obj == ScreenGui or obj == hubMarker then
            return
        end
    ...
    for _, child in pairs(CoreGui:GetChildren()) do
        if child.Name ~= "Kryzen.net" and child.Name ~= "KryzenDupeGUI" then
            ScanUI(child)
        end
    end
